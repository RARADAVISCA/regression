...
```
# A. Analisis Konsumsi Heating Oil
...
## 3. Scatter Plot Heating_Oil vs Avg_Age
```{r}
...
## 4. Scatter Plot Temperature vs Insulation
```{r}
...
# B. Model Prediksi Heating Oil (Sarah)
...
# C. Prediksi Calon Pelanggan Heating Oil (Marketer)
## 1.  Membaca Model dari Sarah
```{r}
# Membaca model yang sudah dibuat dan disimpan oleh Sarah
model <- readRDS("model_heating_oil.rds")

```
## 2. Membaca Data Calon Pelanggan
```{r}
...
```
## 3. Memprediksi Kebutuhan Minyak
```{r}
